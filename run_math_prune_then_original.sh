...
#SBATCH --job-name=qwen3-math-ablate
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
AFLOW_MAX_ROUNDS="${AFLOW_MAX_ROUNDS:-20}"
...
AFLOW_PRUNED_OPTIMIZED_PATH="${AFLOW_PRUNED_OPTIMIZED_PATH:-workspace_math_binomial_ucb_20}"
AFLOW_ORIGINAL_OPTIMIZED_PATH="${AFLOW_ORIGINAL_OPTIMIZED_PATH:-workspace_math_original_20}"
...
echo "Pruned optimized path: ${AFLOW_PRUNED_OPTIMIZED_PATH}"
echo "Original optimized path: ${AFLOW_ORIGINAL_OPTIMIZED_PATH}"
...
seed_aflow_workflow() {
    local optimized_path="$1"
    local dataset="$2"
    local source_workflow_dir="${SOURCE_WORKFLOW_DIR:-workspace/${dataset}/workflows}"
    local target_workflow_dir="${optimized_path}/${dataset}/workflows"
    ...
    touch "${optimized_path}/__init__.py"
    mkdir -p "${optimized_path}/${dataset}"
    touch "${optimized_path}/${dataset}/__init__.py"
...
mkdir -p logs "${AFLOW_PRUNED_OPTIMIZED_PATH}" "${AFLOW_ORIGINAL_OPTIMIZED_PATH}"
VLLM_LOG="${REPO_DIR}/logs/vllm_${SERVED_MODEL_NAME}_math_ablate.log"
touch "${VLLM_LOG}"
...
run_math_aflow() {
    ...
    seed_aflow_workflow "${optimized_path}" "${AFLOW_DATASET}"
    ...
    python -u run.py \
        ...
        --enable_eb_ucb_early_stop "${enable_pruning}" \
...
run_math_aflow "${AFLOW_PRUNED_OPTIMIZED_PATH}" true "MATH 20-round run with Clopper-Pearson pruning"
run_math_aflow "${AFLOW_ORIGINAL_OPTIMIZED_PATH}" false "MATH 20-round original run without pruning"

echo "AFlow MATH ablation finished."
